<script lang="ts">
	...
	import {
		socket,
		SOCKET_REQUEST_MANUALLY_MOVE_AUTHORITIES_UNITS_TOPIC
	} from '$root/utils/socketio';
	import { unitTypeIconMap } from '$root/utils/unitTypeUtils';

	$: selectedUnitsAmountOptions = Array.from(
		{ length: unitStatus?.amount ?? 0 },
		(_, i) => '' + (i + 1)
	);
	let initialSelectedUnitsAmount = 1;
	let selectedUnitsAmount = initialSelectedUnitsAmount;

	$: selectedMoveToLocationOptions = Object.values(Location)
		.filter((l) => l !== unitStatus?.location)
		.map(enumValueToString);
	$: selectedMoveToLocationOptionsIcons = Object.values(Location)
		.filter((l) => l !== unitStatus?.location)
		.map((l) => locationIconMap[l]);
	...
	const onUnitsAmountDropdownSelectionChange = (option: string) =>
		(selectedUnitsAmount = Number(option));
...
<div class="w-full flex flex-col py-2 lg:py-4">
	...
	<div class="flex flex-wrap justify-start items-center">
		{#if unitStatus && unitStatus.amount > 0}
			...
			/>
		{:else if unitStatus}
			No Units of type {unitTypeIconMap[unitStatus?.unitType]}
			{enumValueToString(unitStatus?.unitType)} currently positioned at {locationIconMap[
				unitStatus?.location
			]}
			{enumValueToString(unitStatus?.location)}.
		{/if}
	</div>
</div>
