<script lang="ts">
	...
	const getUnitsAmountDeltaToPreviousValue = (authoritiesHistoricUnitStatusData: UnitsStatusDataWithRelatedWeatherEventData[], i: number) => {
		if(i == authoritiesHistoricUnitStatusData.length - 1) {
			return authoritiesHistoricUnitStatusData[i].amount;
		} else {
			return authoritiesHistoricUnitStatusData[i].amount - authoritiesHistoricUnitStatusData[i + 1].amount;
		}
	};
					...
					{#each authoritiesHistoricUnitStatusData as unitStatus, i}
						<tr class={"flex w-full text-center" + ((i < authoritiesHistoricUnitStatusData.length - 1) ? " border-b border-accentLight dark:border-accentDark" : "")}>
							...
							<td class="flex w-1/4 justify-center items-center"
								><div
									class={'rounded-full w-fit text-xs md:text-sm text-black px-3 ' + getColorByUnitType(unitStatus.unitType)}
								>
									{unitStatus.amount} Units
								</div>
							</td>
							<td class="flex w-1/4 justify-center items-center">
								<div class={"text-xs md:text-sm ml-3 font-bold " + (getUnitsAmountDeltaToPreviousValue(authoritiesHistoricUnitStatusData, i) >= 0 ? "text-green-400" : "text-red-400")}>
									({getUnitsAmountDeltaToPreviousValue(authoritiesHistoricUnitStatusData, i) >= 0 ? '+ ' : '- '} {Math.abs(getUnitsAmountDeltaToPreviousValue(authoritiesHistoricUnitStatusData, i))})
								</div>
							</td>
							<td class="flex w-1/4 justify-center items-center">
