<script lang="ts">
	...
	export let showInvalidInputAsError = true;
	let additionalClass = '';
...
<div class={additionalClass ? ' ' + additionalClass : ''}>
	...
	{#if !isInputValid && showInvaidInputMessage}
		<p
			class={showInvalidInputAsError
				? 'text-red-300 text-xs italic'
				: 'text-yellow-300 text-xs italic'}
		>
			{invalidInputMessage(currentInput)}
		</p>
	{/if}
</div>
