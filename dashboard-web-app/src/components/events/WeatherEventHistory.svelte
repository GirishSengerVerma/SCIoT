<script lang="ts">
	...
	import {
		formatWeatherEventAction,
		getWeatherEventRiskLevelColor
	} from '$root/utils/weatherEventUtils';
...
<div class="w-full flex flex-col py-1 lg:py-3 justify-evenly">
					...
					{#each historicWeatherEventData.risks as weatherEventRisk, i}
						...
						>
							<td class="w-1/3 py-4 px-2 font-semibold"
								>{dayjs(weatherEventRisk.start).format('DD.MM.YYYY HH:mm:ss')} ({format(
									weatherEventRisk.start,
									'en_US'
								)})</td
							>
							<td class="w-1/3 py-4 px-2 font-semibold"
								>{weatherEventRisk.end
									? dayjs(weatherEventRisk.end).format('DD.MM.YYYY HH:mm:ss') +
									  ' (' +
									  format(weatherEventRisk.end, 'en_US') +
									  ')'
									: 'Still ongoing'}</td
							>
							<td class="flex w-1/3 justify-center items-center"
								><div
									class={'rounded-full w-fit h-fit mx-auto text-xs md:text-sm text-black px-3 ' +
										getWeatherEventRiskLevelColor(weatherEventRisk.riskLevel)}
								>
	...
	<SubTitle
		text={(isPast ? 'Past ' : '') + 'Weather Event Actions History'}
		clazz="mt-4 lg:mt-10 mb-1 lg:mb-3"
	/>
	<div class="w-full border border-accentLight dark:border-accentDark rounded-lg lg:my-2">
		<table class="w-full text-xs lg:text-sm text-left">
			<thead
					...
					{#each historicWeatherEventData.actions as weatherEventAction, i}
						...
						>
							<td class="w-1/2 py-4 px-2 font-semibold"
								>{dayjs(weatherEventAction.timestamp).format('DD.MM.YYYY HH:mm:ss')} ({format(
									weatherEventAction.timestamp,
									'en_US'
								)})</td
							>
							<td class="flex w-1/2 justify-center items-center">
								{weatherEvent
									? formatWeatherEventAction(
											{ ...weatherEventAction, weatherEvent },
											false,
											true,
											false
									  )
